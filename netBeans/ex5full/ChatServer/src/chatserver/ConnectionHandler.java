...
//==============================================================================
    ...
    private  void StartChat(){
        try {
            ...
            int counter = 0;
            while(_sr.getUserName().length()<1 && counter<5){
                try {
                    counter++;
                    Thread.sleep(1000);
            ...
            Integer temp;
                if(_usersNames.containsKey(_sr.getUserName())){
                    ...
                    temp = 1;
                    _data.add("#" + _sr.getUserName() + " # has entered the chat!");
                    _usersNames.put(_sr.getUserName(),temp);
                }
            while(_reader.read() != -1 && counter < 5){
                try {
    ...
    public void Stop(){
        ...
        Integer temp;
        String tempUserName = _sr.getUserName();
        _sr.stop();
        _sw.stop();
        temp = _usersNames.get(tempUserName);
        temp -=1;

        _usersNames.put(tempUserName,temp);   
        if(temp == 0){
             _data.add("#" + tempUserName + "#: has leave the chat.");
            _usersNames.remove(tempUserName);
